  ...
  local script = string.format([[
    tell application "Music"
      ...
      if (count of albumTracks) > 0 then
        -- Disable shuffle to play in album order
        set shuffle enabled to false

        -- Create a temporary playlist to play the album
        -- This ensures all tracks play in sequence
        try
          delete playlist "VinylTempAlbum"
        end try

        set tempPlaylist to make new user playlist with properties {name:"VinylTempAlbum"}

        repeat with t in albumTracks
          duplicate t to tempPlaylist
        end repeat

        -- Set the view to this playlist and play it
        -- This makes it the current playlist context
        try
          set view of front window to tempPlaylist
        end try
        play tempPlaylist
