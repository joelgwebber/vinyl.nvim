			...
			if state_done and queue_done then
				if state_result then
					...
					if M.last_track_id ~= current_track_id then
						...
						if
							M.track_changing
							and M.changing_from_track_id
							and current_track_id ~= M.changing_from_track_id
						then
...
-- Helper to set a keymap (handles string or table of keys)
local function set_keymap(buf, keys, action)
	local opts = { noremap = true, silent = true }
	local rhs = ':lua require("vinyl.ui").' .. action .. "<CR>"
	if type(keys) == "table" then
		for _, key in ipairs(keys) do
			vim.api.nvim_buf_set_keymap(buf, "n", key, rhs, opts)
		end
	elseif keys and keys ~= "" then
		vim.api.nvim_buf_set_keymap(buf, "n", keys, rhs, opts)
	end
end

-- Helper function to set up buffer keymaps (called once per buffer)
local function setup_buffer_keymaps(buf)
	local km = config.options.keymaps and config.options.keymaps.ui or {}

	set_keymap(buf, km.close or { "q", "<Esc>" }, "close()")
	set_keymap(buf, km.play_pause or "<Space>", "action_play_pause()")
	set_keymap(buf, km.next_track or "n", "action_next_track()")
	set_keymap(buf, km.prev_track or "N", "action_previous_track()")
	set_keymap(buf, km.volume_up or "=", "action_increase_volume()")
	set_keymap(buf, km.volume_down or "-", "action_decrease_volume()")
	set_keymap(buf, km.toggle_shuffle or "s", "action_toggle_shuffle()")
	set_keymap(buf, km.seek_backward or "h", "action_seek_backward(5)")
	set_keymap(buf, km.seek_forward or "l", "action_seek_forward(5)")
	set_keymap(buf, km.seek_backward_large or "H", "action_seek_backward(30)")
	set_keymap(buf, km.seek_forward_large or "L", "action_seek_forward(30)")
	set_keymap(buf, km.show_help or "?", "show_help()")
end
...
-- Invalidate queue cache and force a refresh
-- Call this when the playback context has changed (new album, playlist, etc.)
function M.invalidate_queue()
	M.last_queue = nil
	M.last_track_id = nil
	M.track_changing = true

	-- Trigger immediate UI refresh if the UI is open
	if M.win and vim.api.nvim_win_is_valid(M.win) then
		render_ui()
	end
end
